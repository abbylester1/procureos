    ...
    <div className="space-y-12">
      ...
      <div className="grid gap-8 xl:grid-cols-[1.2fr_0.8fr]">
        <div className="surface rounded-3xl p-6">
          <h3 className="text-xl font-semibold tracking-[-0.02em]">Why this recommendation?</h3>
          <p className="mt-4 text-sm leading-6 text-slate-300">BuildTech Supply delivers the strongest procurement outcome: lowest qualified total cost, 98% material coverage, a 32-day delivery timeline, and low execution risk. The recommendation creates {currency(rfq.potentialSavings)} in estimated savings while preserving delivery confidence.</p>
          <div className="mt-6 grid gap-3 md:grid-cols-3">
            <div className="surface-soft rounded-[20px] p-4"><div className="text-sm text-slate-500">Cost advantage</div><div className="mt-2 text-sm font-semibold text-success">Best qualified</div></div>
            <div className="surface-soft rounded-[20px] p-4"><div className="text-sm text-slate-500">Missing items</div><div className="mt-2 text-sm font-semibold">2 low-impact</div></div>
            <div className="surface-soft rounded-[20px] p-4"><div className="text-sm text-slate-500">Approval status</div><div className="mt-2 text-sm font-semibold text-warning">Pending</div></div>
          </div>
        ...
        <div className="surface-muted rounded-3xl p-6">
          <h3 className="text-xl font-semibold tracking-[-0.02em]">Alternative Tradeoffs</h3>
          <div className="mt-4 space-y-3">
            {suppliers.slice(1, 4).map((supplier) => (
              <div key={supplier.id} className="rounded-[20px] bg-white/[0.035] p-4">
                <div className="flex items-center justify-between"><span className="text-sm font-medium">{supplier.name}</span><SupplierPill risk={supplier.risk} /></div>
